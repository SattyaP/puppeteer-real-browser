...
export const startSession = ({ args = [], headless = 'auto', customConfig = {}, proxy = {}, isBuster = false }) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const captcha = "E:/Documents/Kerja/puppeteer-real-browser/src/extension/captcha/";

            if (isBuster) {
                args.push(`--disable-extensions-except=${captcha}`);
                args.push(`--load-extension=${captcha}`);
            }

            const chromeFlags = ['--no-sandbox', '--disable-setuid-sandbox', '--disable-blink-features=AutomationControlled','--window-size=1366,1080'].concat(args);
